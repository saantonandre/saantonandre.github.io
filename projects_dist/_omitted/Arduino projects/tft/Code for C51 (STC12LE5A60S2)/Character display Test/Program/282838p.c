...
sbit	scl		= P1^7;	//SPI卡同步时钟 		接液晶屏CLK引脚
sbit	sda	= P1^5;	//SPI卡同步数据   接到液晶屏SDA/DIN引脚
sbit 	rs        =P1^2;//接液晶屏D/C引脚
sbit 	reset     =P3^3;//接液晶屏RST引脚

//如果想省IO也可以将以下两个控制端接常高电平3.3V
sbit 	cs        =P1^3;//SPI卡片选 接液晶屏CE引脚
sbit 	bl        =P3^2;//接液晶屏BL引脚用于控制背光，也可以接3.3V常亮
//sbit	SPI_MISO	= P1^6;	//SPI卡同步数据 不读取数据此引脚则可不接

...
class Room {
  ...
  draw3() {
    ...
    if (this.tied >= 0) {
      ...
      c.lineTo(
        (rooms[this.tied].pos % mapSize) * size + size / 2,
...
function mapGenerator() {
  ...
  while (rooms.length < roomAmount) {
    ...
    while (flag) {
      if (
        (rooms[rand].pos % mapSize == 0 && rand2 == -1) ||
        ((rooms[rand].pos + 1) % mapSize == 0 && rand2 == +1)
      ) {
    ...
    if (
      rooms[rand].pos + rand2 >= 0 &&
      rooms[rand].pos + rand2 < mapSize * mapSize
    ) {
  ...
  // The treasure room is the furthest away from the boss

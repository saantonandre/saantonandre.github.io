    ...
    for (let i = map.tiles.length - 1; i >= 0; i--) {
        ...
        switch (map.tiles[i].type) {
            ...
            case 9:
                ...
                break;
            case 10: //event box
                entities.push(new EventBox(map.tiles[i].x, map.tiles[i].y, map.tiles[i].w, map.tiles[i].h, map.tiles[i].event, map.tiles[i].repeatable))
                removeList.push(i);
                break;
            case 11: //rock
                entities.push(new DestructibleRock(map.tiles[i].x, map.tiles[i].y))
                removeList.push(i);
                break;
            case 12: //skate
                entities.push(new Skateboard(map.tiles[i].x, map.tiles[i].y))
                entities.push(entities[entities.length - 1].interac)
                removeList.push(i);
                break;
            case 13: //Portal
                entities.push(new Portal(map.tiles[i].x, map.tiles[i].y))
                removeList.push(i);
                console.log("spawnedPortal")
                break;
        }
    }
        debug.log("map initialized, "+entities.length+" entities found");
